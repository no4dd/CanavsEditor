...
export class Draw {
  ...
  public setPageScale(payload: number) {
    ...
    const scrollContainer = typeof this.scrollObserver !== 'undefined' ? 
      this.scrollObserver.getScrollContainer() : document;
    const isDocumentScroll = scrollContainer === document || scrollContainer === document.documentElement;
    
    // Store old scale values to calculate ratios
    const oldScale = this.options.scale;
    const scaleRatio = payload / oldScale;
    const oldHeight = this.getHeight();
    const oldPageGap = this.getPageGap();
    
    // Store cursor-to-viewport position
    ...
    if (cursorPosition) {
      ...
      // Handle multi-page positioning correctly
      const { leftTop } = cursorPosition.coordinate;
      
      // Calculate absolute cursor coordinates with page offset
      // For second page and beyond, we need to calculate the cumulative height of previous pages
      let cumulativePageOffset = 0;
      if (cursorPosition.pageNo > 0) {
        // Calculate offset based on old scale (current scale before change)
        cumulativePageOffset = cursorPosition.pageNo * (oldHeight + oldPageGap);
      }
      
      // Calculate absolute position in viewport coordinates
      const cursorAbsX = containerRect.left + leftTop[0];
      const cursorAbsY = containerRect.top + leftTop[1] + cumulativePageOffset;
      
      // Get position relative to viewport
      cursorViewportX = cursorAbsX - window.scrollX;
      cursorViewportY = cursorAbsY - window.scrollY;
    }
    ...
    this.pageList.forEach((p, i) => {
      ...
      p.style.marginBottom = `${pageGap}px`;
    ...
    // Adjust scroll position to keep cursor in view at the same relative position
    if (cursorPosition) {
      // Recalculate the container position after scaling
      const containerRect = this.container.getBoundingClientRect();
      
      // For second page and beyond, recalculate the cumulative height with new scale
      let newCumulativePageOffset = 0;
      if (cursorPosition.pageNo > 0) {
        newCumulativePageOffset = cursorPosition.pageNo * (height + pageGap);
      }
      
      // Calculate the new cursor absolute position after scaling
      const newCursorAbsX = containerRect.left + (cursorPosition.coordinate.leftTop[0] * scaleRatio);
      const newCursorAbsY = containerRect.top + (cursorPosition.coordinate.leftTop[1] * scaleRatio) + 
                           newCumulativePageOffset;
      
      // Calculate new scroll position to keep cursor in same relative viewport position
      const newScrollX = newCursorAbsX - cursorViewportX;
      const newScrollY = newCursorAbsY - cursorViewportY;
