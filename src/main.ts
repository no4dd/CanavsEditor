    ...
    if (!commentDom.querySelector('.comment-header')) {
      ...
      collapseAllBtn.onclick = () => {
        commentDom.querySelectorAll<HTMLDivElement>('.comment-item').forEach(item => {
          ...
          minimizedComments.add(item.getAttribute('data-id')!)
        })
      }
    ...
    for (const comment of commentList) {
      ...
      if (groupIds.includes(comment.id)) {
        ...
        if (!activeCommentDom) {
          ...
          if (minimizedComments.has(comment.id)) {
            commentItem.classList.add('minimized')
          }
          commentItem.setAttribute('data-id', comment.id)
          commentItem.onclick = (evt) => {
            // 只有点击评论本身，而不是控制按钮时才定位
            if (evt.target === commentItem || 
                !['i', 'BUTTON'].includes((evt.target as HTMLElement).tagName)) {
          ...
          // 创建动作区域：包含最小化/最大化和关闭按钮
          const commentItemActions = document.createElement('div')
          commentItemActions.classList.add('comment-item__actions')
          
          // 最小化/最大化按钮
          const toggleDom = document.createElement('i')
          if (minimizedComments.has(comment.id)) {
            toggleDom.classList.add('maximize')
          } else {
            toggleDom.classList.add('minimize')
          }
          toggleDom.onclick = (evt) => {
            evt.stopPropagation()
            const commentItem = commentDom.querySelector<HTMLDivElement>(
              `.comment-item[data-id='${comment.id}']`
            )
            if (commentItem) {
              if (commentItem.classList.contains('minimized')) {
                commentItem.classList.remove('minimized')
                toggleDom.classList.remove('maximize')
                toggleDom.classList.add('minimize')
                minimizedComments.delete(comment.id)
              } else {
                commentItem.classList.add('minimized')
                toggleDom.classList.remove('minimize')
                toggleDom.classList.add('maximize')
                minimizedComments.add(comment.id)
              }
            }
          }
          ...
          closeDom.onclick = (evt) => {
            evt.stopPropagation()
            instance.command.executeDeleteGroup(comment.id)
          }
          
          commentItemActions.append(toggleDom)
          commentItemActions.append(closeDom)
          commentItemTitle.append(commentItemActions)
  ...
  instance.listener.rangeStyleChange = function (payload) {
    ...
    if (payload.groupIds) {
      ...
      if (activeCommentDom) {
        ...
        // 如果评论是最小化的，展开它
        if (activeCommentDom.classList.contains('minimized')) {
          activeCommentDom.classList.remove('minimized')
          minimizedComments.delete(id)
          const toggleDom = activeCommentDom.querySelector<HTMLElement>('.maximize')
          if (toggleDom) {
            toggleDom.classList.remove('maximize')
            toggleDom.classList.add('minimize')
          }
